...
public class DeleteAssignmentUseCase {
    ...
    @Transactional
    public void execute(Long assignmentId, Long userId) {
        log.debug("Deleting assignment {} by user {}", assignmentId, userId);
        
        Assignment assignment = assignmentRepository.findById(assignmentId)
                .orElseThrow(() -> new AssignmentNotFoundException(assignmentId));
        
        if (!assignment.isOwnedBy(userId)) {
            log.error("User {} attempted to delete assignment {} owned by {}", 
                    userId, assignmentId, assignment.getTeacherId());
            throw new UnauthorizedAccessException("Csak a saját feladataidat törölheted");
        }
